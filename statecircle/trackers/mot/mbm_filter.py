...
Created on Mon Dec 23 16:44:46 2019
...
class MBMFilter(MultiObjectFilter):
    r""" Poisson multi-Bernoulli mixture filter

    The assumed density is a multi-Bernoulli mixture(MBM) RFS
    """

    def __init__(self, surviving_rate, prob_min, prob_estimate, *args, **kwargs):
        super(MBMFilter, self).__init__(*args, **kwargs)
        ...
        # each leaf is a Bernoulli state in MBM filter, contains a existence
        ...
        # in MBM filter, the other is Poisson state.
        # self.mbm = MultiBernoulliMixtureState(np.empty(0), [])

    def Bern_predict(self, Bern, time_step, inplace=False):
        # NOTE: inplace op
        # Bern performs prediction step for a Bernoulli component
        if not inplace:
            Bern = deepcopy(Bern)
    ...
    def Bern_detected_update_lik(self, Bern, meas):
        ...
        # TODO: DEBUG with the `-np.log(intensity_c)` term
    ...
    def Bern_detected_update_state(self, Bern, meas):
        ...
        Bern_detected_upd = BernoulliState(prob=1., state=self.density_model.update(Bern.state, meas, self.measurement_model))
        return Bern_detected_upd

    def reduction(self):
        # removes Bernoulli components with small probability of existence, 
        # and re-index the hypothesis table. If a track contains no single object hypothesis after pruning,
        # this track is removed.
    ...
    def birth(self, meas_data):
        # TODO: birth_model in MBM filter should be a Multi-Bernoulli State, 
        # TODO: not a Poisson state
        # birth bernoulli components
        birth_state = self.birth_model.birth().intensity # GaussianSumState
        self.hypo_forest += [[BernoulliState(prob, state)] for prob, state in \
                             zip(birth_state.weights, birth_state.gaussian_states)]
    ...
    def update(self, meas_data):
        ...
        # 1) Bernoulli update. For each Bernoulli state density, create a missed detection
        ...
        # ellipsoidal gate of the giving state density
        num_trees = len(self.hypo_forest)
        ...
        for h in range(num_global_hypo):
            ...
            for i in range(num_trees):
                # TODO: fatal BUG! the case for hypo_table[h, i] == -1!
                # assert hypo_table[h,i] != -1
                # workaround for now: extract valid_rows in cost_mat according to
                # the hypo_table.
            ...
            # DEBUG with `add_back_lik`
            add_back_lik = 0
            #            for i in range(num_local_tree):
            #                if self.hypo_table[h,i] >= 0:
            #                    add_back_lik += assoc_lik[i][self.hypo_table[h,i], 0]
        ...
        # 5) prune local hypotheses (or hypothese trees) that do not appear in 
    ...
    def prune_hypo_table(self, log_w, hypo_table, hypo_forest):
        ...
        for i in range(len(tt_update)):
            ...
            if np.any(local_hypo_idx == -1):
                ...
                tt_update[i] = list_index(tt_update[i], keep_local_idx.astype(np.int_))
                tmp = ht_update[:, i]
    ...
    def estimate(self):
        # performs object state estimation in the MBM filter
        ...
        # proabilit is above a threshold
        estimate = []
