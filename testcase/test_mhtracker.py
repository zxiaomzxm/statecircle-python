...
Created on Sat Sep 12 19:51:11 2020
...
from statecircle.trackers.mot.pmbm_tracker import PMBMTracker
from statecircle.trackers.mot.meas_driven_pmbm_tracker import MeasDrivenPMBMTracker

PMBMTracker = MeasDrivenPMBMTracker # [PMBMTracker | MeasDrivenPMBMTracker]
seed = 666
# build scene
scene = LinearScenario.caseF(birth_weight=0.0005, birth_cov_scale=400)
...
measurement_model = LinearMeasurementModel(mapping=[1, 1, 0, 0],
                                           sigma=10)
clutter_model = PoissonClutterModel(detection_rate=1.0,
                                    lambda_clutter=20,
...
# build sensor model
no_clutter_model = PoissonClutterModel(detection_rate=1.0,
                                        lambda_clutter=0,
                                        scope=[[-1000, 1000], [-1000, 1000]])

sensor_model = DummySensorModel(no_clutter_model, measurement_model, random_seed=seed, noisy=True)
...
density_model = KalmanAccumulatedDensityModel(traceback_range=1)
...
# reductor
reductor = HypothesisReductor(weight_min=0.01, merging_threshold=4, capping_num=1)

# %% build trackers & filtering
# some extra parameters
# TODO: reformat the input parameters
prior_birth = False
surviving_rate = 0.99
recycle_threshold = 0.1
prob_min = 0.01
prob_estimate = 0.5
meas_models_dict = None
clutter_models_dict = None

pmbm_filter = PMBMTracker(prior_birth,
                        surviving_rate,
                       recycle_threshold,
                       prob_min,
                       prob_estimate,
...
pmbm_estimates = pmbm_filter.filtering(data_reader)
...
for k, (_, obj_meas_data, clutter_data) in enumerate(data_reader.truth_meas_generator()):
    ...
    PMBM_estimated_state = np.hstack(pmbm_estimates[k]).squeeze()
...
PMBM_card_pred = []
for step, tracks in enumerate(pmbm_estimates):
    valid_track_num = 0
    for track in tracks:
        range_t = track['range'][-1] - track['range'][0] + 1
